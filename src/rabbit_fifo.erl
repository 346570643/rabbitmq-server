...
apply(_, #checkout{spec = {dequeue, unsettled},
                   meta = ConsumerMeta, consumer_id = {_, {_, Pid}} = ConsumerId},
      State0) ->
    State1 = update_consumer(ConsumerId, ConsumerMeta,
                             {once, 1, simple_prefetch}, State0),
...
apply(_, {nodeup, Node}, #state{consumers = Cons0,
    ...
    Cons = maps:fold(fun({_, {_, P}}, #consumer{suspected_down = true}, Acc)
                           when node(P) =:= Node ->
                             [P | Acc];
                        (_, _, Acc) -> Acc
                     end, [], Cons0),
    Enqs = maps:fold(fun({_, P}, #enqueuer{suspected_down = true}, Acc)
                           when node(P) =:= Node ->
                             [P | Acc];
                        (_, _, Acc) -> Acc
                     end, [], Enqs0),
...
active_locators(#state{enqueuers = Enqs,
                       consumers = Cons}) ->
    lists:usort(maps:keys(Enqs) ++ [L || {_, L} <- maps:keys(Cons)]).
...
query_consumers(#state{consumers = Consumers}) ->
    maps:map(fun ({Tag, {_, Pid}}, #consumer{meta = Meta}) ->
                     {Pid, Tag,
